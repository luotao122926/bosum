...
import com.bosum.erp.api.DeptApi;
import com.bosum.erp.api.vo.UserDeptIdsVO;
    ...
    public Mono<Void> check(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if (StrUtil.isNotEmpty(userId)) {
            try {
                ...
                ResultData<UserDeptIdsVO> resultData = deptApi.getDeptIdsByUserId(userId);
                if (ObjectUtil.isNotNull(resultData)) {
                    if (resultData.isSuccess()) {
                        UserDeptIdsVO userInfo = resultData.getData();
